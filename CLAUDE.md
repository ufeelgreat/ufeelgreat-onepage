...
5. Parcours ("D'où je viens 📍") — timeline chiffrée (80+ assets/mois, reel viral 13,5 M vues ; montants € des mandats retirés, confidentiels)
...
2. Film Cancun
3. Productions institutionnelles | France (carousel, 4 vignettes `portfolio-france-*.webp` fournies par Gaël, Monet en vignette principale ; 4 liens : Monet/Normandie Impressionniste, Royal de Luxe/Le Havre, MuMa, Je suis Rouen — corrobore les mandats de la timeline)
4. Émission Twitch (carousel autoplay)
5. Snaaake The Game
6. Mango RX (lien YouTube)
7. Let's Learn | Coulisses d'une pub (id `letslearn`, lien sur demande — ne JAMAIS nommer FansRevenue ni les marques adultes sur le site)
8. Prototype IA

Astuce vignettes YouTube : `https://img.youtube.com/vi/VIDEO_ID/maxresdefault.jpg` (repli `hqdefault.jpg`), puis conversion webp 1254px.
...
- [x] Chiffres validés dans la timeline : 180 personnes, équipe de 4, 80+ assets/mois, 4-6 campagnes, reel viral 13,5 M vues (montants € des mandats retirés depuis, confidentiels)
...
### Phase 7 refonte UX — Effet WOW Apple-style (2026-07-15, branche claude/onepage-ux-phase-4-6-dwa5zx) ✓
- [x] Hero : parallax avatar porté à 46px (tilt 3D souris + glare testés puis retirés, 16 juil — effet non souhaité)
- [x] Sorties de scène scroll-linked généralisées : chaque section (sauf Contact) recule et s'estompe en quittant le viewport (`--exit-p` par section via initExitScenes, CSS sur `.section > .container`)
- [x] Marks timeline recentrés (16 juil) : Crakmedia = "workflows IA déployés" (plus "entreprise de 180 personnes"), Aktua = "coordination de projets institutionnels", Arcanes = "projets audiovisuels de bout en bout"
- [x] Accordion "L'humain derrière le CV" : 1 phrase clé `<mark>` par bloc (extra), surlignée 500 ms après l'ouverture (post dé-floutage) ; rejouée à chaque réouverture ; bloc initial ouvert swept au scroll-in ; marks des blocs fermés exclus d'initMarkSweep
- [x] Optimisation recruteur (16 juil) : ligne d'intitulés de postes sous le titre hero (`hero.roles`) ; CTA "Me contacter →" magnétique vers #contact ; 5ᵉ bullet hero PEMTL (118 inscrits) ; carte PEMTL enrichie (Spotify previews, notifications, trophées, admin, 118 inscrits en mark) ; balises OG/Twitter + og-image.jpg 1200×630 (générée headless, script scratchpad)
- [x] Expertise refondue en grille flip 3D (remplace les piles, Phases 11/18 obsolètes) : 14 cartes, mots-clés visibles côté face, détail au dos (rotateY au clic), filtres par thème réactivés ("Tous" + 3), hauteur uniforme JS, cascade au scroll-in ; initPileSwipe et CSS piles supprimés
- [x] Filtre Expertise animé (FLIP via WAAPI) : sortantes en clone fantôme fondu+scale, restantes glissent vers leur nouvelle position, entrantes en fondu+scale décalé, hauteur de grille animée
- [x] Portfolio : carte PEMTL vedette (`.portfolio-card--featured`, halo coral pulsant 3,2 s, coupé au hover, statique en reduced-motion) ; ordre Twitch ↔ Cancun inversé (Twitch en 2ᵉ) ; JSON réindentés par le swap Python (diff cosmétique)
- [x] Portfolio bento : PEMTL, Twitch, Productions France et Prototype IA en `--wide` (span 2) — grille 4×3 pleine à 1024px (chaque rangée = 1 large + 1 simple), vignette large en absolu pour que la hauteur de rangée vienne des cartes simples 16/9 ; à 768px (2 col) les larges passent pleine largeur en 21/9 ; mobile inchangé
- [x] Vision : illumination progressive du texte scroll-linked type Apple (mots `.vw` allumés un à un, bidirectionnel ; wrapping par TreeWalker préservant les `<mark>`)
- [x] `<mark>` : surlignage balayé coral→orange au scroll-in (`.swept` via IntersectionObserver ; dans Vision, déclenché par l'illumination) — `background-color: transparent` requis pour neutraliser le jaune UA
- [x] Compteurs timeline : gradient text coral→orange, taille 1.7rem, pop d'atterrissage
- [x] Cascades d'apparition : timeline + témoignages, enfants un à un (délais JS purgés après la cascade) ; réinjection langue gérée via `data-cascaded`
- [x] Tilt portfolio renforcé : ±9° + scale 1.04 + glare
